...
export default function ContentHub() {
  ...
  useEffect(() => { listPublishedPosts({ homeOnly: true }).then((posts) => { setLivePosts(posts); setNeedLogin(false); }).catch(() => { setLivePosts([]); setNeedLogin(true); }).finally(() => setLoading(false)); }, []);
  ...
  return <div className="site-shell">
    <header className="site-header"><div className="header-inner"><Link className="brand" href="/" aria-label="Posid AI담당관3.0 홈"><span className="brand-logo-wrap"><img src="/brand/posid-ci-02.jpg" alt="PoSID" /></span><span className="brand-divider" /><span className="brand-service">AI담당관 <b>3.0</b></span></Link><nav className="desktop-nav" aria-label="주요 메뉴">{categoryNav.map((slug) => <Link key={slug} href={`/category/${slug}`}>{categories[slug].label}</Link>)}</nav><a className="header-search-link" href="#stories"><SiteIcon name="search" size={19} /><span>글 찾기</span></a><Link className="header-write-link" href="/write">글쓰기</Link><UserMenu /></div></header>
    <main>
      {needLogin ? <section className="section-wrap"><div className="empty-state"><strong>로그인이 필요합니다.</strong><p>글을 읽으려면 카카오 계정으로 로그인해 주세요.</p><a className="primary-button" href="/api/auth/kakao/login">카카오로 로그인하기</a></div></section> : <>
