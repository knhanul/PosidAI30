...
const emptyEditor: EditorState = {
  category: "news", title: "", summary: "", body_markdown: "", content_format: "html", topics: [], key_points: [],
          ...
          {!editor ? (
            ...
            <form className="admin-editor" onSubmit={save}>
              ...
              <aside className="editor-side-fields">
                ...
                <section><h3>분류</h3><label><span>카테고리</span><select value={editor.category} onChange={(event) => update("category", event.target.value as CategorySlug)}>{Object.entries(categories).map(([key, category]) => <option value={key} key={key}>{category.label}</option>)}</select></label><label><span>토픽</span><input value={topicInput} onChange={(event) => setTopicInput(event.target.value)} placeholder="회의, 업무자동화" /><small>쉼표로 구분합니다.</small></label></section>
