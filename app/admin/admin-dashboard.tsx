...
import {
  buildShortPostPayload, createPost, deleteAttachment, deletePost, getMe, listAdminPosts, logout, setFeatured, unsetFeatured, updatePost,
  uploadAttachments, uploadThumbnail, uploadInlineImage, uploadNewPostInlineImage, uploadInlineVideo, uploadNewPostInlineVideo, unlinkKakao, type AdminUser, type ApiPost, type PostPayload,
} from "../api-client";
import { categories, shortCategoryLabels, shortCategoryLabel, type PostCategorySlug, type ShortCategory } from "../content";
import RichTextEditor from "../rich-text-editor";

type EditorState = PostPayload & { id?: string };
const emptyEditor: EditorState = {
  category: "news", short_category: null, external_url: null, title: "", summary: "", body_markdown: "", content_format: "html", content_density: "normal", topics: [], key_points: [],
...
function fromApi(post: ApiPost): EditorState {
  return {
    id: post.id, category: post.category, short_category: post.short_category, external_url: post.external_url, title: post.title, summary: post.summary,
    body_markdown: post.category === "short" ? (post.body_markdown ?? "") : post.content_format === "html" ? (post.body_markdown ?? "") : `<p>${(post.body_markdown ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, "</p><p>")}</p>`, content_format: post.content_format, content_density: post.content_density === "compact" ? "compact" : "normal", topics: post.topics, key_points: post.key_points, is_featured: post.is_featured, show_on_home: post.show_on_home,
...
export default function AdminDashboard() {
  ...
  function changeCategory(category: PostCategorySlug) {
    setEditor((current) => current ? { ...current, category, short_category: category === "short" ? current.short_category ?? "tip" : null, external_url: category === "short" ? current.external_url : null } : current);
  }

  async function save(event: React.FormEvent) {
    event.preventDefault(); if (!editor) return;
    if (editor.category === "short" && !editor.short_category) { setError("짧게보기 분류를 선택해 주세요."); return; }
    if (editor.category !== "short" && imageUploading) { setError("이미지 업로드가 끝난 후 저장해 주세요."); return; }
    if (editor.category !== "short" && /src=["'](?:data:image|blob:)/i.test(editor.body_markdown)) { setError("본문 이미지 업로드가 완료되지 않았습니다. 이미지를 다시 삽입해 주세요."); return; }
    setBusy(true); setMessage(""); setError("");
    try {
      const topics = topicInput.split(",").map((item) => item.trim().replace(/^#/, "")).filter(Boolean).slice(0, 10);
      const payload: PostPayload = editor.category === "short"
        ? buildShortPostPayload({ title: editor.title, body: editor.body_markdown, shortCategory: editor.short_category!, externalUrl: editor.external_url, topics, showOnHome: editor.show_on_home })
        : { ...editor, short_category: null, external_url: null, content_format: "html", topics, key_points: keyPointInput.map((item) => item.trim()).filter(Boolean).slice(0, 3), is_featured: editor.is_featured && editor.show_on_home };
      let saved = editor.id ? await updatePost(editor.id, payload, csrf) : await createPost(payload, csrf);
      if (editor.category !== "short" && thumbnail) saved = await uploadThumbnail(saved.id, thumbnail, csrf);
      if (editor.category !== "short" && attachments.length) await uploadAttachments(saved.id, attachments, csrf);
        ...
        <main className="admin-main">
          ...
          {!editor ? (
            <section className="admin-list-panel"><div className="admin-panel-title"><div><span>POSTS</span><h2>전체 글</h2></div><button className="admin-primary" onClick={startNew}>새 글 작성</button></div>
              <div className="admin-post-table"><div className="admin-table-head"><span>글</span><span>홈 노출</span><span>조회</span><span>수정일</span><span>관리</span></div>{posts.map((post) => <article key={post.id}><div><span>{post.category === "short" ? shortCategoryLabel(post.short_category) : categories[post.category].label}</span><strong>{post.title}</strong><small>/{post.slug}</small></div><b className={`status-chip ${post.show_on_home ? "published" : "draft"}`}>{post.is_featured ? "대문 글" : post.show_on_home ? "홈 표시" : "홈 미표시"}</b><span className="view-count-cell">{(post.view_count ?? 0).toLocaleString("ko-KR")}</span><time>{new Date(post.updated_at).toLocaleDateString("ko-KR")}</time><div><button onClick={() => startEdit(post)}>수정</button>{post.category !== "short" && <button className={post.is_featured ? "danger" : ""} disabled={busy} onClick={() => toggleFeatured(post)}>{post.is_featured ? "대문 해제" : "대문 지정"}</button>}<button className="danger" onClick={() => remove(post)}>삭제</button></div></article>)}</div>
            ...
            <form className="admin-editor" onSubmit={save}>
              ...
              <div className="editor-grid"><section className="editor-main-fields">
                ...
                {editor.category === "short" ? <label className="short-editor-field"><span>내용 *</span><textarea value={editor.body_markdown} onChange={(event) => update("body_markdown", event.target.value)} required maxLength={2000} rows={10} placeholder="AI 팁, 발견, 활용 아이디어나 메모를 짧게 적어 주세요." /><small>{editor.body_markdown.length.toLocaleString("ko-KR")} / 2,000</small></label> : <>
                  ...
                  <label><span>본문 *</span><RichTextEditor value={editor.body_markdown} onChange={(value) => update("body_markdown", value)} onUpload={async (file) => editor.id ? (await uploadInlineImage(editor.id, file, csrf)).url : (await uploadNewPostInlineImage(file, csrf)).url} onUploadVideo={async (file) => editor.id ? (await uploadInlineVideo(editor.id, file, csrf)).url : (await uploadNewPostInlineVideo(file, csrf)).url} onUploadingChange={setImageUploading} disabled={busy} /><small>문서처럼 작성하면 허용된 HTML로 저장됩니다. 첫 글 작성부터 사진과 동영상을 추가할 수 있습니다.</small></label>
              ...
              <aside className="editor-side-fields">
                <section><h3>게시 설정</h3><label className="check-label"><input type="checkbox" checked={editor.show_on_home} onChange={(event) => { const checked = event.target.checked; update("show_on_home", checked); if (!checked) update("is_featured", false); }} /><span>홈에 표시</span></label><small>해제해도 글은 카테고리와 검색 결과에서 계속 볼 수 있습니다.</small>{editor.category !== "short" && <><label className="check-label"><input type="checkbox" checked={editor.is_featured} disabled={!editor.show_on_home} onChange={(event) => update("is_featured", event.target.checked)} /><span>대표 글로 표시</span></label><small>홈에 표시된 글 중 하나를 첫 화면의 큰 글로 보여줍니다.</small></>}</section>
                <section><h3>분류</h3><label><span>카테고리</span><select value={editor.category} disabled={Boolean(editor.id && editor.category === "short")} onChange={(event) => changeCategory(event.target.value as PostCategorySlug)}>{editor.id ? (editor.category === "short" ? <option value="short">짧게보기</option> : Object.entries(categories).map(([key, category]) => <option value={key} key={key}>{category.label}</option>)) : <>{Object.entries(categories).map(([key, category]) => <option value={key} key={key}>{category.label}</option>)}<option value="short">짧게보기</option></>}</select></label>{editor.category === "short" && <label><span>짧게보기 분류 *</span><select value={editor.short_category ?? "tip"} onChange={(event) => update("short_category", event.target.value as ShortCategory)}>{Object.entries(shortCategoryLabels).map(([key, label]) => <option value={key} key={key}>{label}</option>)}</select></label>}<label><span>해시태그</span><input value={topicInput} onChange={(event) => setTopicInput(event.target.value)} placeholder="AI/보안/모델, 업무자동화" /><small>쉼표로 구분합니다. /로 계층을 표현합니다 (예: AI/보안/모델).</small></label>{editor.category === "short" && <label><span>외부 링크 (선택)</span><input type="url" value={editor.external_url ?? ""} onChange={(event) => update("external_url", event.target.value || null)} placeholder="https://" /></label>}</section>
                {editor.category !== "short" && <section><h3>대표 이미지</h3><label className="radio-label"><input type="radio" name="thumbnail" checked={editor.thumbnail_type === "preset"} onChange={() => update("thumbnail_type", "preset")} /><span>카테고리 기본 이미지</span></label><label className="radio-label"><input type="radio" name="thumbnail" checked={editor.thumbnail_type === "webdav"} onChange={() => update("thumbnail_type", "webdav")} /><span>직접 업로드</span></label>{editor.thumbnail_type === "webdav" && <input type="file" accept="image/png,image/jpeg,image/webp" capture="environment" onChange={(event) => setThumbnail(event.target.files?.[0] ?? null)} />}{selectedPost?.thumbnail_url && <img className="admin-thumbnail" src={selectedPost.thumbnail_url} alt="현재 대표 이미지" />}<div className="home-card-preview"><span>홈 카드 미리보기</span><strong>{editor.title || "제목을 입력해 주세요"}</strong><p>{editor.summary || "요약이 홈 화면에 표시됩니다."}</p></div></section>}
