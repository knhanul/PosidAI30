...
import { createPublicPost, getMe, getUserPostForEdit, updatePublicPost, uploadInlineImage, uploadNewPostInlineImage, uploadPublicThumbnail, type AuthState, type PostPayload } from "../api-client";
...
export default function WritePage() {
  ...
  const [form, setForm] = useState({ category: initialCategory, title: "", summary: "", body: emptyBody, contentDensity: "normal" as "normal" | "compact", thumbnailType: "preset" as "preset" | "webdav", topics: "" });
  ...
  async function submit(event: React.FormEvent) {
    ...
    const payload: PostPayload = { category: form.category, title: form.title, summary: form.summary, body_markdown: form.body, content_format: "html", content_density: form.contentDensity, topics: form.topics.split(/[, ]+/).map((item) => item.trim().replace(/^#/, "")).filter(Boolean).slice(0, 10), key_points: [], is_featured: false, show_on_home: true, thumbnail_type: form.thumbnailType, service_status: null, service_audience: null, service_url: null };
    try {
