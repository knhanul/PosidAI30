...
import { categories, shortCategoryLabels, type Post } from "./content";
...
export default function PostDetail({ slug, fallback }: { slug: string; fallback?: Post }) {
  ...
  if (post.category === "short") return <div className="short-detail-page"><SiteHeader /><main><article className="short-detail"><nav className="breadcrumb" aria-label="현재 위치"><Link href="/">홈</Link><span>/</span><Link href="/shorts">짧게보기</Link></nav><div className="short-detail-meta"><span className="short-category-badge">{post.shortCategory ? shortCategoryLabels[post.shortCategory] : "짧게보기"}</span><time>{post.date}</time></div><h1>{post.title}</h1><p className="short-detail-body">{post.bodyMarkdown || post.summary}</p>{!!post.topic.length && <div className="topic-row article-topics">{post.topic.map((topic) => <Link key={topic} href={`/shorts?q=${encodeURIComponent(`#${topic}`)}`}>#{topic}</Link>)}</div>}{post.externalUrl && <a className="short-external-link" href={post.externalUrl} target="_blank" rel="noopener noreferrer">원문 보기 →</a>}{post.ownedByCurrentUser && post.id && <Link className="article-edit-link" href={`/write?edit=${post.id}`}>내 짧게보기 수정</Link>}</article>{renderCommunity()}<div className="short-detail-back"><Link href="/shorts">← 짧게보기</Link></div></main></div>;
