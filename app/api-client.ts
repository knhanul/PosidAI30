...
export async function unsetFeatured(id: string, csrf: string) { return apiFetch<void>(`/api/admin/posts/${id}/featured`, { method: "DELETE", headers: { "X-CSRF-Token": csrf } }); }
...
export async function uploadInlineVideo(id: string, file: File, csrf: string) { const body = new FormData(); body.append("file", file); return apiFetch<{ url: string }>(`/api/posts/${id}/inline-videos`, { method: "POST", headers: { "X-CSRF-Token": csrf }, body }); }
export async function uploadNewPostInlineVideo(file: File, csrf: string) { const body = new FormData(); body.append("file", file); return apiFetch<{ url: string }>("/api/inline-videos", { method: "POST", headers: { "X-CSRF-Token": csrf }, body }); }
